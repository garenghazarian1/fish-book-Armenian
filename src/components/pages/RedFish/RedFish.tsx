...
export default function RedFish() {
  ...
  const autoplayStep = (currentIndex: number) => {
    ...
    autoplayTimeoutRef.current = setTimeout(() => {
      ...
      autoplayTimeoutRef.current = setTimeout(() => {
        const nextIndex = (currentIndex + 1) % redMoods.length;
  ...
  const handleScroll = (dir: "next" | "prev") => {
    ...
    setIndex((prev) =>
      dir === "next"
        ? (prev + 1) % redMoods.length
        : (prev - 1 + redMoods.length) % redMoods.length
    );
    ...
    <div className={styles.swipeContainer} ref={containerRef}>
      ...
      <AnimatePresence mode="wait" custom={direction}>
        ...
        >
          ...
          <audio ref={audioRef} src={redMoods[index].audio} preload="auto" />
        </motion.div>
      </AnimatePresence>
